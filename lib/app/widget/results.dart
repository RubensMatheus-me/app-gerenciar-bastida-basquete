...
class _ResultsState extends State<Results> with TickerProviderStateMixin {
  ...
  Future<void> _loadTotals() async {
    ...
    for (var team in widget.teams) {
      ...
      for (var player in teamPlayers) {
        final playerPoints = await statsDao.getTotalPointsByPlayer(player.id, widget.matchId);
        final playerErrors = await statsDao.getTotalErrorsByPlayer(player.id, widget.matchId);
    ...
    return Padding(
      ...
      child: ListView(
        children: [
          ...
          const Text('Resumo Geral da Partida', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
          const SizedBox(height: 8),
          _buildSummaryChart(),
          const SizedBox(height: 8),
          Text('Aproveitamento total: ${_totalEfficiency.toStringAsFixed(1)}%', style: const TextStyle(fontSize: 16, color: Colors.white)),
          const SizedBox(height: 16),
          const Text('Desempenho dos Jogadores', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
          const SizedBox(height: 8),
                  ...
                  BarChartData(
                      ...
                      touchTooltipData: BarTouchTooltipData(
                        getTooltipItem: (group, groupIndex, rod, rodIndex) {
                          return BarTooltipItem(
                            rodIndex == 0 ? 'Pontos: ${points}' : 'Erros: ${errors}',
                            TextStyle(
                              color: rodIndex == 0 ? Colors.green : Colors.redAccent,
                              fontWeight: FontWeight.bold,
                            ),
                          );
                        },
                    ...
                    barGroups: [
                      BarChartGroupData(
                        x: 0,
                        barRods: [
                          BarChartRodData(toY: points.toDouble(), color: Colors.green, width: 18, borderRadius: BorderRadius.circular(6)),
                          BarChartRodData(toY: errors.toDouble(), color: Colors.redAccent, width: 18, borderRadius: BorderRadius.circular(6)),
                        ],
                        barsSpace: 6,
                      ),
                    ],
  ...
  double maxY = data.map((e) => e.points + e.errors).reduce((a, b) => a > b ? a : b) * 1.2;
  double interval = 10;
  while ((maxY / interval) > 5) {
    interval += 10;
  }
          ...
          BarChartData(
            ...
            barGroups: data.asMap().entries.map((entry) {
              ...
              return BarChartGroupData(
                x: idx,
                barRods: [
                  BarChartRodData(toY: d.points.toDouble(), color: Colors.green, width: 18, borderRadius: BorderRadius.circular(6)),
                  BarChartRodData(toY: d.errors.toDouble(), color: Colors.redAccent, width: 18, borderRadius: BorderRadius.circular(6)),
                ],
                barsSpace: 6,
              );
            }).toList(),
  ...
  Widget _buildPlayerPerformanceChart(List<PlayerStatisticsSummaryDto> results) {
          ...
          child: LineChart(
            LineChartData(
                ...
                touchTooltipData: LineTouchTooltipData(
                  getTooltipItems: (touchedSpots) {
                    return touchedSpots.map((spot) {
                      ...
                      return LineTooltipItem('$playerName\n${spot.y.toInt()} pts', TextStyle(color: playerColors[playerName], fontWeight: FontWeight.bold));
                    }).toList();
                  },
                ),
              ),
              ...
              titlesData: FlTitlesData(
                ...
                leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, interval: 1, getTitlesWidget: (v, meta) => Text(v.toInt().toString(), style: const TextStyle(color: Colors.white)))),
              ),
      ...
      children: widget.teams.map((team) {
        ...
        return Padding(
          ...
          child: Card(
            ...
            child: ExpansionTile(
              title: Text(team.name, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.amber)),
...
class TeamEfficiencyData {
  ...
  final Color color;
  TeamEfficiencyData({required this.teamName, required this.points, required this.errors, required this.efficiency, required this.color});
}
