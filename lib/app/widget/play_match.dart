...
class _PlayMatchState extends State<PlayMatch> {
  ...
  Future<void> _onCellTapped(int points, int cellIndex, Offset globalTapPosition) async {
    setState(() {
  ...
  print('Clique na célula: index=$cellIndex, row=$row, col=$col, zone=$zone, pontos=$points, acerto=$hit');

  final assertivenessPitchId = hit ? 1 : 2;

  if (widget.match.id == null) {
    throw Exception("ID da partida está nulo. A partida deve ser inserida antes.");
  }
...
bool _isThreePointCell(int cellIndex) {
  final color = highlightedCells[cellIndex];
  if (color == null) return false;

  return color.withAlpha(255) == Colors.yellow;
}
  ...
  if (pointsA >= maxPoints || pointsB >= maxPoints) {
    String winnerName = pointsA >= maxPoints ? widget.teamA.name : widget.teamB.name;
...
ZoneType _determineZoneFromCell(int index) {
  ...
  void _initializeHighlights() {
    ...
    for (int col = 0; col < cols; col++) {
      highlightedCells[(rows - 1) * cols + col] = Colors.yellow.withValues(alpha: 0.5);

    }
    for (int row = 0; row < rows; row++) {
      highlightedCells[row * cols] = Colors.yellow.withValues(alpha: 0.5);
      highlightedCells[row * cols + (cols - 1)] = Colors.yellow.withValues(alpha: 0.5);
    }

    // parte azul da quadra
    int middleStart = (cols ~/ 2) - 1;
    for (int offset = 0; offset < 3; offset++) {
      highlightedCells[middleStart + offset] = Colors.blue.withValues(alpha: 0.5);
    }
  ...
  Future<void> _registerScore(int points, int cellIndex) async {
    if (_selectedPlayer == null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('+$points - ${_selectedPlayer!.name}', style: const TextStyle(fontWeight: FontWeight.bold)),
          duration: const Duration(seconds: 1),
        ),
      );
  ...
  Widget _buildFloatingButtons() {
    ...
    final position = _buttonPosition ?? Offset(screenSize.width / 2, screenSize.height / 2);
  ...
  return Container(
    ...
    child: ListView.builder(
      ...
      itemCount: _isTeamAVisible ? _playersTeamA.length : _playersTeamB.length,
      itemBuilder: (context, index) {
        final player = _isTeamAVisible ? _playersTeamA[index] : _playersTeamB[index];
...
Widget _buildGridButton(int row, int col, double buttonWidth, double buttonHeight) {
  ...
  Color cellColor = highlightedCells[cellIndex] ??
      (points == 3 ? Colors.yellow.withValues(alpha: 0.5) : Colors.red.withValues(alpha: 0.5));
